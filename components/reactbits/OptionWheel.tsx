...
/**
 * OptionWheel — adapted from React Bits (https://reactbits.dev/components/option-wheel)
 * Labels sit on a curved wheel and ease toward a target index on scroll/drag/keys;
 * a single rAF loop lays every item out along the curve based on its distance
 * from the current position, so transform/opacity/blur all stay in step.
 */
...
type OptionWheelProps = {
  ...
  soundVolume?: number;
  className?: string;
  /** Element to capture wheel scroll on instead of the wheel's own root — lets a
...
export default function OptionWheel({
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // At the first/last option (non-looping), stop trapping the wheel so the
      // page can keep scrolling past this section instead of getting stuck.
      const atMax = !cfg.loop && step > 0 && current >= cfg.count - 1 - 1e-6;
      const atMin = !cfg.loop && step < 0 && current <= 1e-6;
      if (atMax || atMin) return;
      e.preventDefault();
      applyTarget(current + step, false);
